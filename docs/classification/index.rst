...
The covid-ht project encapsulates all the classification logic in the ``CurrentClassifier`` object. This object performs the classification at all parts of the instance and it is configured by its admin interface (``/admin/base/currentclassifier``).
...
If enabled by **Network Voting Policy**, the Classificatin service will request to other network's node the classification of the data requested to the instance.
